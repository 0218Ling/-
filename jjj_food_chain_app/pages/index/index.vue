<template>
	<view :data-theme="theme()" :class="theme() || ''">
		<diy style="position: relative;" :diyItems="items" @scanQrcode="scanQrcode" ref="diyRef"></diy>
		<!--点击收藏-->
...
export default {
	...
	onLoad(e) {
		...
		let s_id = e.shopSupplierId ? e.shopSupplierId : scene.sid;
		if (s_id) {
			...
			self._get(
				...
				function(res) {
					...
					if (uni.getStorageSync('selectedId') == '') {
						uni.setStorageSync('selectedId', res.data.supplier ? res.data.supplier.shopSupplierId : 0);
					}
